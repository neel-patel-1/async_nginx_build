...
#check for build
[ ! -d "${KTLS_DIR}" ] || [ ! -f "${KTLS_DIR}/openssl-3.0.0.tar.gz" ] || [ ! -d "${KTLS_DIR}/openssl-3.0.0" ] || [ ! -f "${KTLS_DIR}/nginx-1.21.4.tar.gz" ] || [ ! -d "${KTLS_DIR}/nginx-1.21.4" ] || [ ! -f "${KTLS_DIR}/nginx_build/sbin/nginx" ] && echo "missing directory" && ${ROOT_DIR}/scripts/ktls/ktls_build.sh >/dev/null
